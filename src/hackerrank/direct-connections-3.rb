...
(0...T).each {|tc|
  ...
  udist = Set.new
  udelta = Set.new
  puts "Starting test case #{tc}: # cities=#{n}"
  (0...n-1).each {|i|
    #puts "Starting #{i}"
    d1, p1 = cities[i]
    upop << p1
    ...
    (i+1...n).each {|j|
      ...
      udelta << (d1 - d2).abs
      dbgBig += 1
      #dbgBig += 1 if (dist + d * p1).class == Bignum
      #dist = (dist + d * p1) % 1_000_000_007
    }
  }
  puts "Bignum count: #{dbgBig}"
  puts "# uniq pops=#{upop.size} dist=#{udist.size} delta=#{udelta.size}"
  puts dist
}
